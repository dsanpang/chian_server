...
def test():
    while 1:
        try:
            ...
            sql = "select address,network from user where network='eth' order by create_time desc" #倒序
            #sql = "select address,network from user where network='eth' order by create_time asc" #正序
            CONN.execute(sql)
            rows = CONN.fetchall()
            for row in rows:
                print(row)
